...
export function useTopModels(params: { startDate?: Date; endDate?: Date, checks?: string }) {
...
export function useTopTemplates(startDate: Date, endDate: Date, checks: string) {
  const timeZone = new window.Intl.DateTimeFormat().resolvedOptions().timeZone
  const checksParam = checks ? `&checks=${checks}` : ""
  const { data, isLoading } = useProjectSWR(
